...
class App(Frame):
    ...
    def upload_new_bom(self, window: tkinter, hot_runner: bool = None):
        """
        ...
        try:
            ...
            define_sheet_name: Callable = lambda: 'HOT_RUNNER' if hot_runner else 'MOLD'
            try:
    ...
    def get_value_by_selected_row(self, table_name: str, column_name: str) -> str:
        ...
        # Получение таблицы
        define_table: Callable = lambda: self.sorted_molds_data_dict.get(self.sort_status) if 'BOM' not in table_name \
            else self.sorted_bom_dict.get(self.sort_status)
        if self.sort_status:
            table = define_table()
        else:
